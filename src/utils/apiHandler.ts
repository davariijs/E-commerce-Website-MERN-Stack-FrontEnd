...
export const handleAddWishlist = async ({ title, image, price, pathname, uid }: TWishlist) => {
  try {
    ...
    const response = await authenticatedFetch(`${process.env.REACT_APP_URL_API}/api/wishlist`, {
      method: 'POST',
      headers: {
...
export const InfoAccountFunc = async (uid: string | null) => {
  try {
    ...
    const result = await authenticatedFetch(
      `${process.env.REACT_APP_URL_API}/api/info-account/${uid}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
export const OrderFunc = async (uid: string | null) => {
  try {
    ...
    const result = await authenticatedFetch(
      `${process.env.REACT_APP_URL_API}/api/check-out/${uid}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
