...
import {
  ...
  selectLoadingState,
  selectMenHoodies,
} from '../../redux/menProducts/menHoodiesSlice/menHoodiesSlice';
...
export default function MenHoodies() {
  ...
  const menHoodies = useSelector((state: RootState) => selectMenHoodies(state));
    ...
    contentToDisplay = (
      <>
        <div className="lg:grid md:grid sm:grid lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-2 flex  justify-center flex-wrap  lg:gap-10 gap-5 h-fit w-full">
          {menHoodies?.payload?.products
            .filter(
              (itemCategory: TProduct) =>
                itemCategory.productTitle !== null &&
                itemCategory.prices[0].regularPrice.minPrice > values[0] &&
                itemCategory.prices[0].regularPrice.minPrice < values[1]
            )
            .map((itemCategory: TProduct) => (
              <CategoriesCard
                onClick={() =>
