        ...
        {orders?.map((order: TOrder) => (
          <div key={order._id}>
            <div className="bg-secondary md:py-7 md:px-12 py-3 px-5 flex justify-between items-center rounded-lg">
              <div>
                <h3 className="text-darkText font-semibold md:text-base text-sm">
                  Order no: #{order._id}
                </h3>
                <h4 className="font-light text-base text-borderGrey pt-2">
                  Placed On {formatDate(order.orderDate ?? new Date().toISOString())}{' '}
                </h4>
              </div>
              <h4 className="font-semibold md:text-base text-sm text-grayText">
                Total : <span className="text-darkText">${order.totalPrice}</span>
              </h4>
            </div>
            ...
            {order?.cartItems?.map(item => (
              <div
                key={item._id}
                className="bg-secondary md:py-7 md:px-12 py-3 px-5 my-10 rounded-lg"
              >
                <div className="flex justify-between font-semibold md:text-base text-sm py-5 border-b border-borderGrey">
                  ...
                  <h4 className="text-darkText md:text-sm text-xs">
                    Qty <span className="text-grayText font-light">{item.quantity}</span>
                  </h4>
