...
} from '../../redux/productDetails/productDetails';
...
export default function ProductDetails() {
  ...
  const handleFetchProduct = React.useCallback(() => {
    const productId = `${productIdDetails}`;
    dispatch(fetchProductDetails(productId));
  }, [productIdDetails, dispatch]);
  ...
  const handleAddToCart = () => {
    ...
    if (productsDetails?.payload?.products[0].swatchImages.length !== 0) {
      if (!colorProduct) {
        ...
        <div className="container mx-auto">
          <div>
            <div className="lg:grid grid-cols-2 gap-6">
                ...
                {videoUrl ? (
                  ...
                  <div>
                    <img
                      ...
                      src={productsDetails?.payload?.products[0].images[0].url}
                      alt="color"
                    />
                  </div>
                )}
              ...
              <div>
                <h3 className="text-grayText font-medium text-lg mb-5 mt-5">
                  {productsDetails?.payload?.products[0].brand}
                </h3>
                <div>
                  <h2 className="text-darkText font-bold text-4xl mb-5">
                    {productsDetails?.payload?.products[0].productTitle}
                  </h2>
                </div>
                <div className="flex mb-5">
                  <StarRatings
                    ...
                    starSpacing="8px"
                  />
                  <span className="text-grayText font-medium text-lg pl-2">
                    {productsDetails?.payload?.products[0].avgRating}
                  </span>
                </div>
                <div className="text-grayText font-medium text-lg mb-5">
                  <a className=" flex" href={sizeChartURL}>
                    Size guide <GoArrowRight className="mt-1 w-10" />
                  </a>
                </div>
                <div className="text-darkText font-semibold text-lg mb-5">Colors Available </div>
                <div className="grid grid-cols-6 gap-2 mb-6">
                  {productsDetails?.payload?.products[0].swatchImages?.map(color => (
                    <div key={color.URL}>
                      <div
                      ...
                      >
                        <img className="rounded-full" src={color.URL} alt="color" />
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex lg:justify-start justify-center mb-9">
                  <button
                    ...
                    className="rounded-lg flex w-52 h-12 bg-primary font-semibold text-lg text-white justify-center items-center"
                  >
                    <span className="pr-2">
                      <CartIcon />
                    </span>
                  ...
                  <button className="ml-6 border-darkText rounded-lg w-36 h-12 text-darkText font-bold border-2 bg-transparent text-center">
                    ${productsDetails?.payload?.products[0]?.price.regularPrice.minPrice}
                  </button>
                </div>
                <hr />
                <div className="grid grid-cols-2 gap-6 mt-10 mb-5">
                  <div className="flex font-medium text-darkText text-lg">
                    <span className="-mt-2 pr-2 bg-secondary w-10 h-10 rounded-full  p-3 mr-2">
                      <img src={creditCardIcon} alt="cart" />
                    </span>
                    Secure payment
                  </div>
                  <div className="flex font-medium text-darkText text-lg">
                    <span className="-mt-2 pr-2 bg-secondary w-10 h-10 rounded-full p-3 mr-2">
                      <img src={fitIcon} alt="size" />
                    </span>
                    Size & Fit
                  </div>
                  <div className="flex font-medium text-darkText text-lg">
                  ...
                  <div className="flex font-medium text-darkText text-lg">
                    <span className="-mt-2 pr-2 bg-secondary w-10 h-10 rounded-full p-3 mr-2">
                      <img src={shippingIcon} alt="Returns" />
                    </span>
